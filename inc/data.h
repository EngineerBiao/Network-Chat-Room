 ...
 * 数据库的表结构如下
 * Usrname(int)  Password(text)  fd(int)  register(int)
 * 
 * 数据库相关函数
 * 各数据的作用：用户名和密码用来登录，然后在线文件描述符用于判断该用户是否在线并对它进行操作，是否注册过用来判断用户注册时是否重名
 * 补充说明
 *  文件描述符：返回-1表示用户不在线，>0表示用户在线且值为其套接字的文件描述符
 *  是否注册过：返回0表示用户名没注册过，1表示用户名注册过（-1则是执行错误）
 */
...
int user_if_online(int name); // 判断用户是否在线，在线返回1，否则返回-1
